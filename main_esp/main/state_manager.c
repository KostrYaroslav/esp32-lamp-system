...
void state_manager_init(void) {
    ...
    ESP_LOGI(TAG, "✅ State Manager успешно инициализирован");
}
...
void state_manager_execute(uint32_t mask, bool set) {
    ...
    if (!espnow_dialog_with_controller(mask, set)) {
        ESP_LOGE(TAG, "❌ Диалог с Управляющим не удался");
    }
}
    ...
    if (xSemaphoreTake(state_lock_mutex, pdMS_TO_TICKS(500)) == pdTRUE) {
        ...
        if (espnow_request_state() && xSemaphoreTake(state_ready_sem, pdMS_TO_TICKS(300)) == pdTRUE) {
            ...
            if (diff != 0) {
                ...
                ESP_LOGI(TAG, "✅ Отмена импульса: все целевые лампы уже в состоянии %s", is_set ? "ON" : "OFF");
            }
        } else {
        ...
        ESP_LOGE(TAG, "🚨 Команда отклонена: шина управления занята");
    }
}
